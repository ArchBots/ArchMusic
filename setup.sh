...
packages(){
	...
	if ! command -v ffmpeg &>/dev/null; then
		...
		if sudo apt install ffmpeg -y &>/dev/null;then
			...
			pprint "FAIL.\n\n" "cred"
			pprint "You need to install ffmpeg manually in order to use ArchMusic, exiting..\n" "cblue"
			exit 1
		fi
	fi
...
repo(){
	...
	[[ ! "ArchMusic" == $(basename -s .git `git config --get remote.origin.url`) ]] &&
	git clone https://github.com/ArchBots/ArchMusic &&  cd ArchMusic
}
...
clear
pprint "Welcome to Arch Music Bot Setup Installer\n\n"
...
repo
installation
pprint "\n\n\n\n\nArch Music Bot Installation Completed!" "cgreen"
sleep 4
clear
...
OWNER_ID = $ownid""" > .env
clear
pprint "\n\n\nYour Vars have been saved Successfully!, Thanks for using ArchMusic Installer, now you can proceed by starting the bot with bash start!"
pprint "\n\n\nWant more vars?"
