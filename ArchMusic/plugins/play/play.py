    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            try:
                ...
                await mystic.edit_text(_["call_11"])
                return await app.send_message(config.LOG_GROUP_ID, _["call_12"])
            except Exception as e:
    ...
    if str(playmode) == "Direct":
        if not plist_type:
            if details["duration_min"]:
                ...
                return await mystic.edit_text(
                    _["play_15"], reply_markup=InlineKeyboardMarkup(buttons)
                )
        try:
            await stream(
                _, mystic, user_id, details, chat_id, user_name,
...
async def play_playlists_command(client, CallbackQuery, _):
    ...
    try:
        await stream(
            ...
            spotify=spotify, forceplay=ffplay,
        )
    except Exception as e:
        return await mystic.edit_text(_err(e, _))
    return await mystic.delete()
